...
<Callout type="info" title="Windows launchers">
For Windows users, `headroom init` and `headroom install apply` create Headroom Dashboard launchers in the Desktop and Start Menu by default. Add `--no-dashboard-shortcuts` if you do not want those launchers.
</Callout>
