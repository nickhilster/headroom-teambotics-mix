...
| `HEADROOM_COMPRESSION_TIMEOUT_SECONDS` | Compression pipeline timeout in seconds for proxy compression stages. Use this when large Codex/OpenAI payloads intermittently fail with `compression_refused` due to timeout. Non-positive/invalid values are ignored. | `45` |
| `HEADROOM_WS_FAIL_OPEN_ON_COMPRESSION_FAILURE` | Compression-failure policy switch for WS and analogous HTTP paths. Default fail-closed protects against forwarding oversized uncompressed payloads; set to `1`/`true` only as an explicit compatibility fallback. | `off` |
| `HEADROOM_WS_COMPRESSION_FAIL_THRESHOLD_BYTES` | Oversize threshold used by compression-failure policy when deciding fail-closed vs passthrough on non-timeout errors. | `262144` |
