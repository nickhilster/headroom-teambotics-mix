...
On Windows, this flow also creates Headroom Dashboard launchers in the Desktop and Start Menu by default. Use `--no-dashboard-shortcuts` to opt out.
...
For Windows users, launcher installation is part of that initial install experience by default so non-technical users can open the dashboard without using a terminal.
